...
final class PhotoLibraryPickerViewController: UIViewController {
    ...
    fileprivate lazy var collectionView: UICollectionView = { [self] in
        $0.dataSource = self
        $0.delegate = self
    ...
    func checkStatus(completionHandler: @escaping ([PHAsset]) -> ()) {
        ...
        case .notDetermined:
            ...
            Assets.requestAccess { [self] status in
        ...
        case .denied, .restricted:
            ...
            alert.addAction(title: "OK", style: .cancel) { [self] action in
    ...
    func fetchPhotos(completionHandler: @escaping ([PHAsset]) -> ()) {
        Assets.fetch { [self] result in
            ...
            case .error(let error):
                ...
                alert.addAction(title: "OK") { [self] action in
                    self.alertController?.dismiss(animated: true)
                }
                alert.show()
            }
